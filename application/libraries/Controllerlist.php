		...
		foreach(glob(APPPATH . 'controllers/*') as $controller) {
			...
			if(is_dir($controller)) {
				...
				foreach(glob(APPPATH . 'controllers/'.$dirname.'/*') as $subdircontroller) {
					...
					$modified_subdircontrollername=$dirname.'/'.$subdircontrollername;
					//echo $modified_subdircontrollername."-------<br>";
					...
					foreach($aMethods as $method) {
						if($method != '__construct' && $method != 'get_instance' && $method != $subdircontrollername) {
							//$aUserMethods['path'][$count] = $dirname.'/'.$subdircontrollername.'/'.$method;
							//$aUserMethods['methods'][$count] = $method;
							$aUserMethods[$count] = $dirname.'/'.$subdircontrollername.'/'.$method;
							$count++;
						}
					}
					//$this->setControllerMethods($subdircontrollername, $aUserMethods);					 					
					$this->setControllerMethods($modified_subdircontrollername, $aUserMethods);					 					
				}
			}
				...
				if(is_array($aMethods)){
					foreach($aMethods as $method) {
						if($method != '__construct' && $method != 'get_instance' && $method != $controllername) {
							//$aUserMethods[] = $method;
							$aUserMethods[] = $controllername.'/'.$method;
						}
					}
				}
